...
function validateGoogleAuth() {
  ...
  if (!clientSecret) {
    ...
    log.warning(
      `GOOGLE_CLIENT_SECRET seems too short (${clientSecret.length} chars) - may be invalid`
    );
  } else {
...
async function runAllTests() {
  try {
    ...
    if (allTestsPassed) {
      console.log(
        `${colors.green}✨ All validations passed! Your environment is ready.${colors.reset}`
      );
      ...
      console.log(
        `${colors.red}You may not be able to run the app until these are resolved.\n${colors.reset}`
      );
      process.exit(1);
    }
  } catch (error) {
